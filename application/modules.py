...
def clean_dataset(df,target):
    ...
    if target in object_cols:
        ...
        category=[c for c in range(len(categories))]
        category_label=[c for c in categories]
        df1[target].replace(to_replace=category_label,value=category,inplace=True)
    df1=pd.concat([df1[numerical_cols],df1[target]],axis=1)
...
def manual(newxv,target,df):
    l=[]
    lor=LogisticRegression()
    df1=clean_dataset(df,target)
    ...
    l.append('Prediction LogisticRegression  : {}'.format(output(logi[0],target,df)))
    l.append("Prediction Score : {}".format(score))
    return l


def predict(listt,target,df):
    abc=manual(listt,target,df)
    return abc

    
def output(arg,target,df):
    object_cols=object_col(df)
    if target in object_cols:
        categories=df[target].value_counts().index
        category=[c for c in range(len(categories))]
        category_label=[c for c in categories]
        return category_label[arg]
    else:
        return arg
