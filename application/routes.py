...
from .modules import calldata,datacollect,pagination,correlation,pairplots,numerical_col,object_col,graphs,predict,clean_dataset
from datetime import datetime
...
def login():
    ...
    form =authForm()
    return render_template("login.html",form=form,link="Don't Have Account ?",title="Login")
        ...
        if request.method=="POST":
            ...
            try:
                for i in request.form:
                    ...
                    if request.form[i]=="target":
                        target=i
                        continue

                    if request.form[i]=="None":
                        continue
                    listt.append(float(request.form[i]))

                print(listt)
                
                pred=predict(listt,target,df)
                return render_template("predict.html",column=titles,zip=zip,pred=pred)
            except:
                flash("Internal Sever error","error")
                return redirect("/")

        if request.method=="GET":
            titles=[c for c in df.columns]
            return render_template("predict.html",column=titles,zip=zip,pred=[])
